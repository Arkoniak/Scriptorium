...
- **EPUB assembly via Pandoc — concrete (design 2026-06-26).** Pandoc reads HTML directly — no need to round-trip Markdown into it. **Canonical consolidation output = HTML** (not Markdown): EPUB is XHTML internally, so HTML→EPUB is the most faithful path; HTML carries everything precisely (emphasis `<i>`/`<b>`, structure `<h1>`/`<p>`, captioned figures `<figure><img><figcaption>`, footnotes, scene-break `<hr>`), whereas Markdown needs extensions and has flavor ambiguity. We mix model formats (Surya HTML, others Markdown) → consolidation normalizes to one HTML. **Pandoc's role is the EPUB packaging** (OPF manifest, `nav.xhtml`/TOC, spine, `container.xml`, chapter splitting, image embedding) — fiddly to hand-roll. **Images:** reference by local `src` path (`<img src="images/0.jpg">`); Pandoc finds the file (`--resource-path`), embeds it into the EPUB and rewrites the link automatically — so placement-in-book is just where the tag sits in the HTML reading order (the bbox-proximity attachment from above). The **cover** (kept as raster, not OCR'd — see the image gate) goes via `--epub-cover-image=<page1>.png`, becoming the EPUB cover rather than inline content. Command sketch:
  `pandoc chapters/*.html --to epub3 --metadata-file meta.yaml --epub-cover-image cover.png --resource-path books/output/<book> --split-level=1 --toc --toc-depth=2 -o <book>.epub`. Open decisions: HTML canonical (recommended) vs Markdown; let Pandoc split by `<h1>` (`--split-level`) vs we pre-split into chapter files; footnote representation (Pandoc-style vs `<a epub:type="noteref">`).
- **Metadata from open sources + the book itself (design 2026-06-26).** We already hold the ISBN (the PDF metadata carries `archive.org/details/isbn_9780380758524`; the cover/copyright shows `0-380-75852-0`). Empirically: **Open Library** by ISBN returns title/author/publisher/year/subjects/cover; **Google Books** returned nothing for this old paperback (open APIs are hit-or-miss). And a key catch — Open Library lists only "Diane Duane", **missing co-author Peter Morwood** that the book's own title page has. So: **core fields from the book itself** (we OCR the copyright/title page anyway — the most reliable source: title, both authors, publisher, year, ISBN, LCCN); **enrichment from open sources** by ISBN (genre/subject **tags**, series, cover thumbnail); **description from the book's own back-cover blurb** (authentic; Google had none). Cross-check source vs book; on conflict prefer the book and flag the mismatch. Map to EPUB Dublin Core via Pandoc (`--epub-metadata` / metadata YAML): `dc:title`, `dc:creator`×2, `dc:publisher`, `dc:date`, `dc:identifier` (ISBN), `dc:language`, `dc:subject` (tags), `dc:description`, plus series via EPUB3 `belongs-to-collection`. Deterministic (API + field mapping + string cross-check), no LLM; a small step orthogonal to OCR, keyed off the ISBN and the copyright-page OCR.
...
### Consolidation / consensus across models — deterministic, layered (design 2026-06-26)
- **Deterministic Python, no LLM in the comparison loop.** ROVER and Consensus Entropy are training-free algorithms; the whole point is auditability (a diff shows exactly what was voted/flagged), unlike asking a model to "merge" outputs.
- **Alignment is the shared core.** Both ROVER and Consensus Entropy ride on aligning the N outputs into one transition network — a multiple-sequence alignment of token streams, done progressively (align hyp1+hyp2 → add hyp3 to the composite → …, as in the original ROVER). NULL arcs handle differing lengths: where 3 models have a token and 2 have nothing, that slot is 3 votes for the token vs 2 for deletion. **This is the answer to "some models omit objects" (e.g. a page number): a missing object is a deletion vote at its slot, not a broken object-by-object compare.**
- **Compare in layers, not all at once** (models emit different formats — Surya HTML, others Markdown/plain):
  - *Layer 1 — characters/words (vote here):* normalize all outputs to a flat plain-text token stream (strip markup, keep case — `MINDBLAST` matters), align, vote → best plain text out of all 5.
  - *Layer 2 — inline emphasis (separate consensus):* only the emphasis-capable models (Surya / Qwen3-VL / Gemma) participate; don't let a model that never emits emphasis cast a "no" vote. Do they agree on where italic is?
  - *Layer 3 — blocks / bbox / labels (separate):* only Surya and Unlimited-grounding; geometry cross-reference.
- **Two products of the one alignment:** ROVER → a voted *output* (assembles the right answer from fragments); Consensus Entropy → a *reliability signal* (per-slot vote entropy: low = confident, high = flag for review). On mostly-agreeing body prose the Entropy QA-flag is the primary value (ROVER has little to fix), and it lights up exactly the hard regions.
- **Granularity:** word-level voting primary; character-level on divergent slots (lets the vote reassemble a word from correct letters across models).
- **Open research angles:** tie-breaks (2-2-1) via char-level sub-vote / preferred model / review flag; vote weighting (we lack per-token confidence except Surya block-level — can Entropy itself weight?); best model *subset* (does a weak model help or hurt the vote?).
- **Tie-breaker: classical OCR (Tesseract/EasyOCR) as a 5th model** (decided 2026-06-29, from the 50/50 tie audit — see `docs/experiments/2026-06-29-surya-spell-checker-bias.md`). The 4-VLM ensemble has 2-vs-2 ties in ~13% of content disagreements; currently resolved by insertion-order (Surya wins). Manual audit shows 35/36 correct, but the 1 failure is structural: non-standard dialect (`Don'` read as `Don't`) where all 4 VLMs have language-model priors pushing toward standard English. Classical OCR has no such priors — it reads glyphs, not words. It is cheap (CPU-only), deterministic, and participates only at tie-breaking slots, not the full pipeline. This is **not** the rejected "classic-OCR-first pipeline" — it is a minimal 5th vote added to break 2-vs-2 deadlocks. Tesseract (mature, well-packaged) is the first candidate; EasyOCR (better on degraded scans) is the alternative.

### Image / graphic gate before consensus — by region, not just page (design 2026-06-26)
- **Decision:** covers and illustrations stay as **raster in the final EPUB** — we do not transcribe them as content. Cover OCR, if any, is only a **QA check** (is this the right book / does the title match?), not body text.
- **The hard regions are stylized display-font (logos), not "cover vs title".** Empirically (the baseline runs): the stylized "Space Cops" logo recurs on the cover (p1), the **title page (p5)**, and the back cover (p260), and *every* model botches it and they disagree — the title page errs **worse** than the cover (Surya "Sprelops", Unlimited "Spade Lops", GLM "SpaceLaps"; Qwen closest). So the divide is the decorative display font (a graphic) wherever it sits, not the page type; the title page's *real* text (authors, publisher) reads fine.
