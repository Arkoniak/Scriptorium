#!/usr/bin/env python3
"""Run Gemma 4 E4B (GGUF) on a book's page images via a llama-server we spawn.

⚠️ EXCLUDED from the ensemble — kept only as the reproducible artifact behind a negative result.
Gemma 4 E4B does NOT reliably OCR dense scanned body text and would poison the consensus: at the
default vision budget it reads only the large header and fabricates the body; raising resolution
(`--image-max-tokens`) needs a matching `--ubatch-size` or llama-server aborts, and even at high
resolution it either loops or produces fluent fabrication. See
docs/experiments/2026-06-26-gemma-vision-failure.md. Not wired into scripts/run_full_book.sh.

Fifth Bag-of-Experts adapter and a second general-purpose VLM (contrast with Qwen3-VL). Same runtime
style as run_qwen3vl.py: spawn llama-server with a vision mmproj, POST each page to the
OpenAI-compatible chat API, collect into the shared normalized run layout, then stop the server.
(The llama-server-driving logic is duplicated from run_qwen3vl.py — a candidate to extract into a
shared helper once a third llama-server model appears.)

See docs/brainstorm.md (Stage 1) and docs/model-comparison.md. The default prompt asks for inline
emphasis (italics/bold) — the axis that splits OCR-specialists from general VLMs.
"""
...
def run_gemma(
    ...
    run_id = f"{utc_now()}__gemma4-e4b__{label}"
